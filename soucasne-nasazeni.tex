...
spolupracuje. Na těch kolejištích je aktuálně nasazeno 99 \gls{mtb} desek.
...
Rovnou uveďme, že komplikovanější systém řízení modelového kolejiště snad nemá
v~Česku a~na Slovensku obdobu. Na první pohled je vidět, že kolejiště je řízeno
dvěma různými systémy a~vyvstává otázka, proč je neintegrovat do systému
jednoho. Taková řešení existují a~mnozí modeláři i~modelářské kluby je
používají. Jsou hojně podporována komerčními softwary, jsou nasazená na velkých
modelových kolejištích například v~Hamburku a~ve Vídni \cite{}. Všechna tato
řešení jsou však o~kompromisech. U~tvorby a nasazení systému \gls{mtb} stály
osoby, které pracují na zabezpečení skutečné železnice.  Systém \gls{mtb} je
tak oproti komerčně dostupným řešením řízení modelových kolejišť navrhován
s~důrazem na spolehlivost a především bezpečnost.
